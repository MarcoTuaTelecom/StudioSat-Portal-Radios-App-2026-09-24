...
for cmd in tar sha256sum nginx systemctl ss ps df free journalctl find; do command -v "$cmd" >/dev/null || { echo "FATAL=MISSING:$cmd" >&2; exit 1; }; done
{
...
for p in radioprincipal radiopop radiorock radioclassicas radiocountry; do
  systemctl status "studiosat-p2@$p.service" --no-pager > "$STATE/status-p2-$p.txt" 2>&1 || true
done
