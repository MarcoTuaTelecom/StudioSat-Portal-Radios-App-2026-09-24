...
on_exit(){ rc=$?; trap - EXIT ERR; if ((rc!=0)); then restore_web; fi; rm -rf "$STAGE" 2>/dev/null || true; exit "$rc"; }
trap on_exit EXIT
...
section "2. HLS.JS OFICIAL LOCAL — ANTES DE MUTAR PRODUCAO"
SIZE="$(stat -c '%s' "$ROOT/assets/vendor/hls.min.js")"
if (( SIZE < 300000 )); then
  echo "HLS_VENDOR=BOOTSTRAP SIZE=$SIZE; baixando bundle oficial 1.7.3"
  bash "$ROOT/scripts/fetch-hls-vendor.sh"
fi
SIZE="$(stat -c '%s' "$ROOT/assets/vendor/hls.min.js")"
(( SIZE > 300000 )) || die "HLS_VENDOR_NOT_PRODUCTION:$SIZE"
sha256sum "$ROOT/assets/vendor/hls.min.js"
python3 "$ROOT/tests/validate_project.py"

section "3. BACKUP COMPLETO OBRIGATORIO"
...
for u in manifest.webmanifest sw.js assets/vendor/hls.min.js assets/js/hls-controller.js assets/icons/icon-192.png; do
